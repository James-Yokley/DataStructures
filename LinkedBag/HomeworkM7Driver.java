...
public class HomeworkM7Driver {
	...
	public static void main(String[] args) {
		...
		testQNotFolowedByU("cinq", true, "q-not-followed-by-u at the end of the word; even length");
	...
	public static boolean qNotFollowedByU(String word) {
		...
		if(word.substring(0,1).toLowerCase().equals("q")){
			if(word.substring(1,2).toLowerCase().equals("u")){
				return false;
			} else{
				return true;
			}
		} else{
			return qNotFollowedByU(word.substring(1, word.length()));
		}
	}
	...
	private static void arrayReverseHelper(int[] array, int first, int last){
		if(first < last){
			int temp = array[first];
			array[first] = array[last];
			array[last] = temp; 
			arrayReverseHelper(array, ++first, --last);
		}
	}

	public static int countPositives(BagInterface<Integer> bag) {
		int count = 0; 
		return countPositivesHelper(bag, count); 
	}
